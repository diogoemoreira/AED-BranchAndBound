...
// AED, 2019/2020
//
// TODO: 93106 Pedro Souto
// TODO: 93321 Luis Pereira
// TODO: 93127 Diogo Moreira
//
...
//      student number of the groupcc -Wall -O2 assignment.c -lm
//   3. (mandatory)
...
static int histograma[3*range*max_n];
...
static void show_solutions(int n,char *header,int what_to_show)
{
  ...
  if((what_to_show & show_histogram) != 0)
  {
    ...
    for(int k=min_cost; k<max_cost+1;k++){
      if(histograma[k]!=0){
        printf("Custo: %d, N: %d\n", k, histograma[k]);
      }
    }
...
static void branch_and_bound(int n,int m,int a[n],int tempcost)
{
  if(m < n - 1){
    int magic_number = 3*(n-m);
    int Magic_number = 60*(n-m);

    if(tempcost+magic_number<min_cost||tempcost+Magic_number>max_cost){   

      for(int i = m;i < n;i++)
      {
#define swap(i,j)  do { int t = a[i]; a[i] = a[j]; a[j] = t; } while(0)
        swap(i,m);                            // exchange a[i] with a[m]
        branch_and_bound(n,m + 1,a,tempcost+(cost[m][a[m]])); // recurse
        swap(i,m);                            // undo the exchange of a[i] with a[m]
#undef swap
      }
    }
...
static void rand_Perms(int N, int n){
...
int main(int argc,char **argv)
{
  if(argc == 2 && argv[1][0] == '-' && argv[1][1] == 'e')
  {
    seed = 0;
    {
      ...
      // escrever o histograma num ficheiro


      if( access("genarray.txt", F_OK ) != -1 ) {
              // file exists
              remove("genarray.txt");
            }
            FILE *fp;
            int sizeu=3*range*max_n;
            fp=fopen("genarray.txt","a");
            for(int i=0;i<sizeu;i++){
              if(histograma[i]!=0){
                fprintf(fp,"%d:%d,",i,histograma[i]);
              }
            }
          fclose(fp);
  ...
  if(argc == 2)
  {
    ...
    if(seed >= 0 && seed <= 1000000)
    {
      for(int n = 1;n <= max_n;n++)
      {
        ...
        // 2.
        //
        if(n <= 12) // use a smaller limit here while developing your code
        {
          int a[n];
          for(int i = 0;i < n;i++)
          ...
          if(n==12){ // Escrever o histograma para n = 12

            if( access("array.txt", F_OK ) != -1 ) {
              // file exists
              remove("array.txt");
            }
            FILE *fp;
        ...
        if(n<=32)
        {
          ...
          reset_solutions();
          (void)elapsed_time();
          branch_and_bound(n,0,a,0);
          cpu_time = elapsed_time();
          show_solutions(n,"Branch and bound",show_info_2 | show_min_solution | show_max_solution);
        }
        //
